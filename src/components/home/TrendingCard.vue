...
<template>
  <div
    ...
    <div v-if="props.loading" class="skeleton-effect flex h-52 w-full items-center justify-center gap-4 p-4">
      <div class="h-36 w-20 flex-shrink-0 rounded bg-gray-300 dark:bg-zinc-800"></div>
      <div class="flex w-full flex-col gap-2">
        <div class="h-3 w-32 rounded-full bg-gray-300 dark:bg-zinc-800"></div>
        <div class="h-2 w-10/12 rounded-full bg-gray-300 dark:bg-zinc-800"></div>
        <div class="w-12/12 h-2 rounded-full bg-gray-300 dark:bg-zinc-800"></div>
        <div class="h-2 w-8/12 rounded-full bg-gray-300 dark:bg-zinc-800"></div>
        <div class="h-2 w-10/12 rounded-full bg-gray-300 dark:bg-zinc-800"></div>
      </div>
    </div>
    <NuxtLink :to="`/details/${props.type}/${props.id}`" v-else
      class="z-0 flex h-52 w-full select-none items-center rounded bg-cover bg-center bg-no-repeat" :style="{
      ...
      <div class="flex items-center justify-center gap-4 p-4">
        <div class="z-10 h-36 w-20 flex-shrink-0 rounded bg-cover bg-center bg-no-repeat drop-shadow-2xl" :style="{
          backgroundImage: `url(https://image.tmdb.org/t/p/w500/${props.poster})`
        }"></div>
        <div class="z-10 !text-white">
          <div class="mb-1 line-clamp-1 font-maven font-semibold leading-4 drop-shadow-2xl">
            {{ props.title }}
          </div>
          <div class="mb-2 line-clamp-4 text-ellipsis text-sm leading-4 opacity-75 drop-shadow-2xl">
            {{ props.overview }}
          </div>
