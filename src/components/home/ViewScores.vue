<template>
  <NuxtLink
    to="/table"
    class="flex flex-col items-center justify-center rounded-3xl bg-gray-50 p-8 drop-shadow-xl transition-all duration-300 ease-in-out hover:bg-gray-100 dark:bg-zinc-900 dark:hover:bg-zinc-800"
  >
    <IconsStarFilled class="h-16 w-16 text-yellow-400" />
    <h1 class="font-maven font-black">
      <span class="text-yellow-400">m</span>aster table
    </h1>
    <p class="text-center text-gray-500 dark:text-gray-300">
      Show the master table
    </p>
  </NuxtLink>
</template>
